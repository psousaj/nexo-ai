...
export class ToolExecutor {
   ...
   * Tool: save_item
   */
  private async saveItemTool(args: {
    type: ItemType;
    title: string;
    metadata?: any;
  }) {
   ...
   * Tool: search_items
   */
  private async searchItemsTool(args: {
    query?: string;
    type?: ItemType | "all";
    limit?: number;
  }) {
   ...
   */
  private async enrichMetadataTool(args: {
  ...
  private async applyTimeoutTool(args: { reason: string }) {
    ...
    console.warn(
      `⏱️ Timeout aplicado: ${this.context.externalId} - Motivo: ${args.reason}`
    );
    ...
    return {
      ...
      reason: args.reason,
      message:
        "Timeout de 5 minutos aplicado. O usuário não poderá enviar mensagens até lá.",
    };
  }
  ...
  private async getStreamingProvidersTool(args: { tmdbId: number }) {
    const providers = await enrichmentService.getStreamingProviders(
      args.tmdbId
    );
