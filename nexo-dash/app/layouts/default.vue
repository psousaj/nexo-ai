...
<template>
	<div class="flex h-screen bg-surface-50 dark:bg-surface-950 transition-colors duration-300">
		...
		<div v-if="isOpen" class="fixed inset-0 bg-black/20 backdrop-blur-sm z-40 lg:hidden transition-opacity" @click="toggleSidebar" />
			...
			>
				<div class="flex items-center gap-4">
					...
					<h1 class="text-xl font-semibold text-surface-900 dark:text-white hidden md:block">
						{{ (route.name?.toString() || 'Nexo AI').replace(/-/g, ' ') }}
					</h1>
				</div>
				...
				<div class="flex items-center gap-3">
					...
					<NuxtLink to="/profile" class="flex items-center gap-3 pl-1 group">
						<div class="text-right hidden sm:block">
							...
							<p class="text-[10px] text-surface-500 font-medium">
