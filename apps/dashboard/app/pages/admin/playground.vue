	...
	<div class="p-6 space-y-6 max-w-5xl">
		...
		<div>
			<h1 class="text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
				<span class="text-3xl">🧪</span> Playground
			</h1>
			<p class="mt-1 text-gray-500 dark:text-gray-400">
				Testes de infra — endpoints, embeddings, conectividade. Só visível para admins.
			</p>
		</div>
				...
				<div class="space-y-4">
					<UFormGroup label="Texto para gerar embedding" help="Mínimo 1 caractere. Textos > 2000 chars serão truncados.">
						<UTextarea
							...
							placeholder="Ex: Inception é um filme de ficção científica de 2010..."
							:rows="4"
							resize
						/>
					</UFormGroup>
					...
					<div class="flex items-center gap-3">
						...
						<span v-if="embeddingText" class="text-xs text-gray-400">
							{{ embeddingText.length }} chars
						</span>
					</div>
					...
					<div v-if="embeddingResult" class="space-y-3">
						...
						<div v-if="!embeddingResult.success" class="space-y-2">
							...
							<div class="bg-red-50 dark:bg-red-900/20 rounded-lg p-3 font-mono text-xs text-red-700 dark:text-red-300 space-y-1">
								<div v-if="embeddingResult.error">
									<span class="text-gray-500">mensagem: </span>{{ embeddingResult.error }}
								</div>
								...
								<div v-if="embeddingResult.code">
									<span class="text-gray-500">código: </span>{{ embeddingResult.code }}
								</div>
						...
						>
							...
							<div class="flex-1 min-w-0 space-y-1">
								<div class="flex items-center gap-2 flex-wrap">
									...
									<UBadge v-if="check.status" color="gray" variant="outline" size="xs">
										HTTP {{ check.status }}
									</UBadge>
