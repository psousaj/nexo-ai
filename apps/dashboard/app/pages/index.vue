...
<template>
	<div class="space-y-8 animate-fade-in">
		...
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
			<!-- Main Growth Chart -->
			<DashboardChartCard v-if="can('manage', 'AdminPanel')" title="Crescimento da Plataforma" class="lg:col-span-2" :loading="isLoading">
				<template #actions>
					<div class="flex items-center gap-1 bg-surface-100 dark:bg-surface-800 p-1 rounded-lg">
						...
						<button class="px-3 py-1 text-xs font-bold rounded-md text-surface-500 hover:text-surface-900">365D</button>
					</div>
				</template>
			...
			<!-- Breakdown Chart -->
			<DashboardChartCard title="Distribuição de Memórias" :class="{ 'lg:col-span-3': !can('manage', 'AdminPanel') }" :loading="isLoading">
		...
		<div class="premium-card">
			<div class="flex items-center justify-between mb-6">
				...
				<NuxtLink to="/memories" class="text-xs font-black text-primary-600 hover:underline uppercase tracking-widest"> Ver tudo </NuxtLink>
			</div>
			...
			<div class="overflow-x-auto">
				<table class="w-full text-left">
					<thead>
						...
						>
							<th class="pb-4 px-4">
								{{ can('manage', 'AdminPanel') ? 'User ID' : 'Origem' }}
							</th>
