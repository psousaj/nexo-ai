...
function normalizeAuthorizationToken(headerValue: string | undefined): string | null {
	if (!headerValue) {
		return null;
	}
...
export function createInternalRuntimeConfigRoutes(options: CreateInternalRuntimeConfigRoutesOptions = {}) {
	const resolveSharedToken = options.resolveSharedToken ?? (() => env.BOTS_CONFIG_PULL_TOKEN);
	const resolvePivotFlags = options.resolvePivotFeatureFlags ?? getPivotFeatureFlags;
	...
	return new Hono().get('/provider-split-config', async (c) => {
		...
		const incomingToken = normalizeAuthorizationToken(c.req.header('authorization'));
