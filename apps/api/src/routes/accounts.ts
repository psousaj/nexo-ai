...
export function registerAccountRoutes(app: Hono) {
	app.get('/user/accounts', async (c) => {
		const accounts = await db.select().from(integrations).where(eq(integrations.userId, c.req.query('userId') ?? 'default'));
		return c.json({ accounts });
	});

	app.post('/user/accounts/sync', (c) => c.json({ success: true, message: 'Sync not implemented', synced: 0, skipped: 0 }));
