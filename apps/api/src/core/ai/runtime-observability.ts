...
export function summarizeRuntimeRounds(rounds: RuntimeRound[]): RuntimeRoundsSummary {
...
export function buildRuntimeObservabilityAttributes(
	...
	if (summary.gatewayHeaders?.cfAigProvider) {
		attributes[`${prefix}.gateway.provider`] = summary.gatewayHeaders.cfAigProvider;
	}
	...
	if (summary.gatewayHeaders?.cfAigEventId) {
		attributes[`${prefix}.gateway.event_id`] = summary.gatewayHeaders.cfAigEventId;
	}

	if (summary.gatewayHeaders?.cfAiReqId) {
		attributes[`${prefix}.gateway.request_id`] = summary.gatewayHeaders.cfAiReqId;
	}
