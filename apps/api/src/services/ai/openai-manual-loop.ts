...
type MinimalTool = Extract<
  ToolName,
  "save_note" | "save_link" | "search_items"
>;

const MINIMAL_TOOL_DEFINITIONS: Record<
  MinimalTool,
...
const SUPPORTED_MINIMAL_TOOLS = new Set<MinimalTool>(
  Object.keys(MINIMAL_TOOL_DEFINITIONS) as MinimalTool[],
);
...
export function buildManualLoopTools(
  availableTools: string[],
): OpenAI.Chat.ChatCompletionTool[] {
  const unsupportedTools = availableTools.filter(
    (toolName) => !SUPPORTED_MINIMAL_TOOLS.has(toolName as MinimalTool),
  );
...
export async function runOpenAIManualLoop(
  ...
  while (rounds < maxRounds) {
    ...
    if (choice?.finish_reason === "length") {
      finalText =
        "A resposta atingiu o limite de tokens. Tente novamente com um pedido mais específico.";
      break;
    }
  ...
  if (!finalText) {
    ...
    finalText =
      "Não consegui concluir a resposta no momento. Tente novamente em instantes.";
  }
