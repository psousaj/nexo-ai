...
export class DeepSeekProvider implements AIProvider {
	...
	constructor(apiKey: string) {
		...
		loggers.ai.info('🔍 DeepSeekProvider configurado');
	}
	...
	async callLLM(
	...
	): Promise<{ round: RuntimeRound; completion: OpenAI.Chat.Completions.ChatCompletion }> {
		...
		try {
			...
			addRuntimeBlock(runtimeRound, {
				type: 'error',
				code: String(error?.statusCode ?? error?.status ?? 'deepseek_error'),
				message: error?.message ?? 'DeepSeek provider error',
