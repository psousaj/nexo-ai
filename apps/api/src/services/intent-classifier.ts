...
export class IntentClassifier {
  ...
  constructor() {
    if (
      ...
      loggers.ai.warn(
        "⚠️ Cloudflare AI Gateway não configurado para Intent Classifier, usando fallback regex",
      );
    } else {
      ...
      loggers.ai.info(
        "✅ Cloudflare AI Gateway configurado para Intent Classifier",
      );
    }
  }
  ...
  async classify(message: string): Promise<IntentResult> {
    return startSpan("intent.classification", async (_span) => {
      ...
      if (!message || typeof message !== "string") {
        ...
        loggers.ai.warn(
          "⚠️ Mensagem inválida para classificação, usando fallback",
        );
      ...
      try {
        ...
        if (regexResult.intent !== "unknown") {
          ...
          loggers.ai.info(
            { intent: regexResult.intent, action: regexResult.action },
            "✅ Matched por regex",
          );
          return regexResult;
        }
   ...
   */
  private mapNeuralToIntentResult(
    neural: IntentAnalysisResult,
    ...
    if (intent === "search_content") {
      ...
      const hasMemoryContext = memoryContextWords.some((kw) =>
        lowerOriginal.includes(kw),
      );
      ...
      query =
        neural.action === "search.all"
          ? undefined
          : (neural.entities?.query ??
            this.extractSearchQuery(originalMessage));
    } else if (intent === "get_info") {
      query = this.extractInfoQuery(originalMessage) || query;
    } else if (action === "delete_item") {
      query = originalMessage
        .replace(
          /deleta|deletar|apaga|apagar|remove|remover|exclui|excluir/gi,
          "",
        )
        .trim();
    }
  ...
  private async classifyWithLLM(message: string): Promise<IntentResult> {
    ...
    try {
      ...
      if (!content) {
        loggers.ai.warn(
          "⚠️ Resposta do Intent Classifier vazia, usando fallback",
        );
      ...
      if (typeof content === "object") {
        ...
        if (!jsonContent.startsWith("{")) {
          loggers.ai.info(
            "⚠️ Resposta não começa com {, tentando extrair JSON...",
          );
  ...
  private classifyWithRegex(message: string): IntentResult {
    ...
    loggers.ai.info(
      { messageSnippet: safeMessage.substring(0, 50) },
      "🎯 Usando fallback regex para classificação",
    );
    ...
    if (this.isDenial(lowerMsg)) {
      loggers.ai.info(
        { intent: "deny", action: "deny", confidence: 0.95 },
        "🎯 Intenção detectada (regex)",
      );
      return {
        intent: "deny",
    ...
    if (this.isInfoRequest(lowerMsg)) {
      loggers.ai.info(
        { intent: "get_info", action: "get_details", confidence: 0.85 },
        "🎯 Intenção detectada (regex)",
      );
      return {
        intent: "get_info",
    ...
    if (this.isSaveRequest(lowerMsg, message)) {
      ...
      const refersToPrevious = saveReferencePatterns.some((pattern) =>
    ...
    if (this.isCasualChat(lowerMsg)) {
      ...
      loggers.ai.info(
        { intent: "casual_chat", action, confidence: 0.8 },
        "🎯 Intenção detectada (regex)",
      );
      return {
        intent: "casual_chat",
    ...
    loggers.ai.info(
      ...
      "🎯 Intenção não identificada (regex)",
    );
    return {
      intent: "unknown",
      action: "unknown",
  ...
  private isConfirmation(msg: string): boolean {
    ...
    if (
      ...
      !deleteKeywords.some((kw: string) => msg.includes(kw))
    ) {
      return true;
    }
  ...
  private isSearch(msg: string): boolean {
    ...
    const hasSearchVerb = searchWithMemoryContext.some((kw) =>
      msg.includes(kw),
    );
    const hasMemoryContext = memoryContextKeywords.some((kw) =>
      msg.includes(kw),
    );
  ...
  private isSaveRequest(msg: string, originalMsg: string): boolean {
    ...
    const isSaveReference = saveReferencePatterns.some((pattern) =>
      pattern.test(msg),
    );
    if (isSaveReference) {
    ...
    const isLongDescription =
      originalMsg.length > 80 &&
      !msg.includes("?") &&
      !this.hasQuestionWords(msg);

    // Se mensagem não é pergunta e menciona conteúdo explicitamente
    const isNotQuestion =
      !msg.startsWith("o que") && !msg.startsWith("qual") && !msg.includes("?");
    ...
    return (
      isLongDescription || (isNotQuestion && hasStreaming && mentionsContent)
    );
  }
  ...
  private isDeleteRequest(msg: string): IntentResult | null {
    ...
    const cleaned = msg
      .replace(
        /deleta|deletar|apaga|apagar|remove|remover|exclui|excluir|limpa|limpar/gi,
        "",
      )
      .trim();
  ...
  private extractItemType(
    msg: string,
  ): "movie" | "tv_show" | "video" | "link" | "note" | "memory" | undefined {
    const typePatterns: Record<
      string,
      "movie" | "tv_show" | "video" | "link" | "note" | "memory"
    > = {
      nota: "note",
      notas: "note",
      lembrete: "note",
  ...
  private extractSelection(msg: string): number | number[] | undefined {
    ...
    const foundNumbers = numberMatches
      ? numberMatches.map((n) => Number.parseInt(n))
      : [];
    ...
    return allSelections.length === 1
      ? allSelections[0]
      : allSelections.sort((a, b) => a - b);
  }
...
// Singleton
export const intentClassifier = instrumentService(
  "intentClassifier",
