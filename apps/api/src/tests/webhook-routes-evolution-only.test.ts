...
describe('Webhook routes - Evolution only', () => {
	test('remove rota legada /webhook/meta e mantém rota oficial Evolution', async () => {
		...
		const legacyGetResponse = await app.request('http://localhost/webhook/meta');
		const legacyPostResponse = await app.request('http://localhost/webhook/meta', {
			method: 'POST',
			headers: { 'content-type': 'application/json' },
			body: JSON.stringify({}),
		});

		const officialEvolutionResponse = await app.request('http://localhost/webhook/whatsapp/evolution', {
			method: 'POST',
			headers: {
