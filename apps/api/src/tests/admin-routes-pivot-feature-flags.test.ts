...
describe('Admin routes - pivot feature flags', () => {
	...
	test('não expõe mais endpoints legados de troca de API e aliases /baileys', async () => {
		...
		const setApiResponse = await app.request('http://localhost/admin/whatsapp-settings/api', {
			method: 'POST',
			headers: { 'content-type': 'application/json' },
			body: JSON.stringify({ api: 'evolution' }),
		});

		const disconnectAliasResponse = await app.request('http://localhost/admin/whatsapp-settings/baileys/disconnect', {
			method: 'POST',
		});

		const restartAliasResponse = await app.request('http://localhost/admin/whatsapp-settings/baileys/restart', {
			method: 'POST',
		});
	...
	test('qr-code não tenta reconectar quando status já está connecting', async () => {
		...
		const app = new Hono().route('/admin', adminRoutes);
		const response = await app.request('http://localhost/admin/whatsapp-settings/qr-code');
		const body = await response.json();
