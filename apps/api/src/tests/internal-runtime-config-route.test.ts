...
function makePivotFlags(overrides?: Partial<PivotFeatureFlags>): PivotFeatureFlags {
	return {
...
describe('internal runtime config route', () => {
	test('returns 503 when shared token is not configured', async () => {
		...
		const response = await app.request('http://localhost/internal/runtime/provider-split-config');
	...
	test('returns 401 when authorization token is missing or invalid', async () => {
		...
		const missingToken = await app.request('http://localhost/internal/runtime/provider-split-config');
		const invalidToken = await app.request('http://localhost/internal/runtime/provider-split-config', {
			headers: {
