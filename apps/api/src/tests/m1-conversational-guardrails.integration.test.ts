...
function simulateGuardrailFlow(params: {
	...
	normalizeAgentResponse(response);

	const isValid = isValidAgentResponse(response);
...
describe('M1 conversational freedom guardrails (integration)', () => {
	...
	test('rejects RESPOND payloads without textual message in runtime validator path', () => {
		const response = parseJSONFromLLM(
			JSON.stringify({
				schema_version: '1.0',
				action: 'RESPOND',
		...
		expect(isValidAgentResponse(response)).toBe(false);
	});
});
