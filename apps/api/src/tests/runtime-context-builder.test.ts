...
const { mockGetHistory, mockGetUserById, mockBuildAgentContext, mockBuildAgentPrompt } = vi.hoisted(() => ({
...
describe('buildRuntimeContext', () => {
	beforeEach(() => {
		...
		mockBuildAgentPrompt.mockImplementation(({ assistantName }: { assistantName: string }) => ({
			system: `SYSTEM:${assistantName}`,
		}));
	});

	it('usa fallback por user quando não há sessionKey', async () => {
		mockGetUserById.mockResolvedValue({ id: 'user-1', assistantName: 'Aurora' });
	...
	it('usa contexto personalizado quando há sessionKey', async () => {
		...
		expect(mockBuildAgentContext).toHaveBeenCalledWith('user-1', 'agent:main:telegram:direct:user-1');
		expect(mockGetUserById).not.toHaveBeenCalled();
		expect(mockBuildAgentPrompt).toHaveBeenCalledWith(
			expect.objectContaining({ assistantName: 'Nexo Prime', availableTools: ['search_items'] }),
		);
		expect(result.systemPrompt).toContain('SYSTEM:Nexo Prime');
		expect(result.systemPrompt).toContain('# PERSONALIZED CONTEXT');
		expect(result.systemPrompt).toContain('Preferências: respostas curtas e objetivas.');
	});
});