...
vi.mock("@/services/user-service", () => ({
  userService: {
    getUserById: vi
      .fn()
      .mockResolvedValue({ id: "test-user-123", assistantName: "NEXO" }),
  },
}));
...
vi.mock("@nexo/otel/tracing", () => ({
  startSpan: vi.fn(
    async (_name: string, fn: (span: unknown) => Promise<unknown>) => fn({}),
  ),
  setAttributes: vi.fn(),
...
vi.mock("@sentry/node", () => ({
  startSpan: vi.fn(
    async (_options: unknown, fn: (span: unknown) => Promise<unknown>) =>
      fn({}),
  ),
...
describe("Clarification Flow (N1/N2)", () => {
  ...
  test("deve tratar escolha inválida (não numérica) sem crash", async () => {
    ...
    vi.spyOn(messageAnalyzer, "classifyIntent").mockRejectedValueOnce(
      new Error("NLP not available"),
    );
