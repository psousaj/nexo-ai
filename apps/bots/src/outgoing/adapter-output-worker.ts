...
export const adapterOutputQueue = new Queue<AdapterOutputQueueJob>(
  "adapter-output",
  {
...
export const adapterOutputDlqQueue = new Queue<AdapterOutputDlqJob>(
  "adapter-output-dlq",
  {
    connection: REDIS_BASE_OPTIONS,
  },
...
function normalizeJobCounts(
  rawCounts: Partial<Record<SnapshotCountKey, number>>,
): AdapterOutputQueueCounts {
  return {
...
export function createAdapterOutputWorker(): Worker<AdapterOutputQueueJob> {
  const worker = new Worker<AdapterOutputQueueJob>(
    ...
    async (job) => {
      ...
      const idempotencyAccepted = await registerIdempotencyKey(
...
export async function shutdownAdapterOutputRuntime(
  worker: Worker<AdapterOutputQueueJob>,
): Promise<void> {
  await Promise.all([
    worker.close(),
