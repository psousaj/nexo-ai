...
export class ItemService {
      ...
      const [existing] = await db
        ...
        .where(
          and(
            ...
            eq(memoryItems.contentHash, contentHash),
          ),
        )
        .limit(1);
  ...
  private prepareTextForEmbedding(params: {
    ...
    if (type === "movie" || type === "tv_show") {
      ...
      if ("director" in tmdbMeta && tmdbMeta.director) {
        ...
        "created_by" in tmdbMeta &&
        tmdbMeta.created_by &&
        tmdbMeta.created_by.length > 0
      ) {
  ...
  async createItem(params: {
    ...
    if (isGlobalType && externalId && provider && metadata) {
      ...
      if (globalItem) {
        ...
        loggers.db.debug(
          { externalId, type },
          "🔗 Linkado com item global existente",
        );
      } else {
        ...
        try {
          const [newGlobal] = await db
            ...
            .onConflictDoNothing({
              target: [
                semanticExternalItems.externalId,
                semanticExternalItems.type,
                semanticExternalItems.provider,
              ],
            })
            .returning();
          ...
          if (newGlobal) {
            ...
            loggers.db.info(
              { externalId },
              "🌍 Item global criado inline (sem vetor, embedding em background)",
            );
            ...
            this.scheduleEmbeddingUpdate({
              ...
              failureMessage:
                "⚠️ Falha ao gerar embedding global em background",
          ...
          loggers.db.warn(
            { err: error },
            "⚠️ Falha ao criar item global, salvando localmente",
          );
        }
      }
    }
  ...
  async listItems(params: { userId: string; type?: ItemType; limit?: number }) {
    ...
    const result = await db
      .select({
        ...
        embedding: sql<
          number[]
        >`COALESCE(${memoryItems.embedding}, ${semanticExternalItems.embedding})`,
        createdAt: memoryItems.createdAt,
        semanticExternalItemId: memoryItems.semanticExternalItemId,
      })
      .from(memoryItems)
      .leftJoin(
        semanticExternalItems,
        eq(memoryItems.semanticExternalItemId, semanticExternalItems.id),
      )
  ...
  async searchItems(params: { userId: string; query: string; limit?: number }) {
    ...
    try {
      ...
      loggers.db.debug(
        { original: query, expanded: expandedQuery.substring(0, 100) },
        "🔍 Query expandida",
      );
      ...
      if (!queryEmbeddingTask.embedding) {
        throw new Error(
          queryEmbeddingTask.block.error || "Falha ao gerar embedding da query",
        );
      }
      ...
      loggers.db.warn(
        { err: error },
        "⚠️ Falha na busca vetorial, usando fallback literal",
      );
    }
  ...
  async advancedSearch(params: {
    ...
    if (genres && genres.length > 0) {
      const genreConditions = genres.map(
        (genre) => sql`${metadataRef}->'genres' @> ${JSON.stringify([genre])}`,
      );
  ...
  async deleteMultipleItems(
    ...
    const result = await db
      .delete(memoryItems)
      .where(
        and(eq(memoryItems.userId, userId), inArray(memoryItems.id, itemIds)),
      )
      .returning();
  ...
  async updateItem(
    itemId: string,
    userId: string,
    updates: { title?: string; metadata?: ItemMetadata },
  ) {
    ...
    if (contentHash !== item.contentHash) {
      try {
        ...
        if (embeddingTask.embedding) {
          ...
          loggers.db.warn(
            { itemId, embeddingTask: embeddingTask.block },
            "⚠️ Falha ao atualizar embedding",
          );
        }
      } catch (error) {
