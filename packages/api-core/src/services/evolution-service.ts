...
export class EvolutionService {
	...
	private async upsertSettings(payload: {
		...
		await db
			...
			.values({
				id: 'global',
				baileysPhoneNumber: payload.phoneNumber,
				baileysConnectionStatus: payload.connectionStatus,
	...
	private async request<T>(
		...
		if (!response.ok) {
			...
			throw new Error(`Evolution API ${method} ${path} failed: ${response.status} ${errorText}`);
		}
	...
	async getInformation(): Promise<Record<string, unknown> | null> {
		return await this.request<Record<string, unknown>>('GET', '/', { acceptNotFound: true });
	}
	...
	async ensureInstanceUpsert(): Promise<EvolutionInstancePayload | null> {
		...
		const created = await this.request<EvolutionInstancePayload>('POST', '/instance/create', { body });
		if (created) {
	...
	async getConnectionState(): Promise<EvolutionConnectionStateResponse | null> {
		const response = await this.request<EvolutionConnectionStateResponse>('GET', `/instance/connectionState/${this.instanceName}`, {
			acceptNotFound: true,
		});
	...
	async connectInstance(number?: string): Promise<EvolutionConnectResponse | null> {
		await this.upsertSettings({ connectionStatus: 'connecting', lastError: null });
	...
	async logoutInstance(): Promise<Record<string, unknown> | null> {
		await this.upsertSettings({ connectionStatus: 'disconnected', lastError: null });
	...
	async bootstrapIfEnabled(): Promise<void> {
		...
		const timeout = new Promise<never>((_, reject) => {
			setTimeout(() => reject(new Error(`Timeout no bootstrap Evolution (${timeoutMs}ms)`)), timeoutMs);
		});
