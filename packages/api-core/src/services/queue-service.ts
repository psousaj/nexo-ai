...
if (!env.REDIS_HOST || !env.REDIS_PASSWORD) {
  throw new Error(
    "Redis não configurado: REDIS_HOST e REDIS_PASSWORD são obrigatórios",
  );
}
 ...
 */
export const enrichmentQueue = new Queue<EnrichmentJob>(
    ...
    return startSpan("queue.close_conversation.process", async (_span) => {
      ...
      try {
        ...
        if (result.length === 0) {
          ...
          queueLogger.warn(
            { conversationId },
            "⚠️ Conversa já foi fechada/cancelada",
          );
          return;
        }
  ...
  async (job) => {
    return startSpan("queue.response.send", async (_span) => {
      ...
      try {
        ...
        queueLogger.info(
          { externalId, attempt: job.attemptsMade + 1 },
    ...
    return startSpan("queue.enrichment.process", async (_span) => {
      ...
      try {
        if (!candidates || candidates.length === 0) {
          ...
          queueLogger.warn(
            { jobId: job.id },
            "⚠️ Nenhum candidato recebido para enrichment",
          );
        ...
        if (newCandidates.length === 0) {
          ...
          queueLogger.info(
            { jobId: job.id },
            "✅ Todos os itens já existem no cache global",
          );
          return {
            inserted: 0,
        ...
        const itemsToInsert = await mapWithConcurrency(
          newCandidates,
        ...
        if (validItems.length === 0) {
          ...
          queueLogger.warn(
            { jobId: job.id },
            "⚠️ Nenhum embedding gerado com sucesso",
          );
          return {
            inserted: 0,
            skipped: candidates.length,
...
enrichmentWorker.on("failed", (job, error) => {
  ...
  queueLogger.error(
    { jobId: job.id, err: error },
    "❌ [enrichment-processing] Job falhou",
  );
...
export async function cancelConversationClose(
  ...
  try {
    ...
    queueLogger.info(
      { conversationId },
      "🔄 Banco atualizado: conversa voltou pra idle",
    );
...
export async function runAwaitingConfirmationTimeoutCron(): Promise<number> {
  try {
    ...
    if (count > 0) {
      queueLogger.info(
        { count },
        "⏰ CRON: Conversas em awaiting_confirmation expiradas fechadas",
      );
    }
    ...
    queueLogger.error(
      { err: error },
      "❌ Erro no timeout de awaiting_confirmation",
    );
    throw error;
  }
}
