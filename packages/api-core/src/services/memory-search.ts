...
async function getEmbedding(text: string): Promise<number[]> {
  ...
  try {
    const { executeEmbeddingTask } =
      await import("@/services/ai/embedding-task");
    ...
    throw new Error(
      embeddingTask.block.error ||
        "Falha ao gerar embedding para memory_search",
    );
  } catch (error) {
    loggers.memory.warn(
      { error },
      "⚠️ Embedding service not available, using placeholder",
    );
...
export async function searchMemory(
  ...
  if (!query || query.trim().length === 0) {
    ...
    loggers.memory.info(
      ...
      "✅ Returning recent items (empty query)",
    );
...
export async function searchDailyLogs(options: {
  ...
  loggers.memory.info(
    { userId, date, query, count: logs.length },
    "✅ Daily logs search complete",
  );
...
export async function upsertDailyLog(options: {
  ...
  const existing = await db.query.agentDailyLogs.findFirst({
    where: and(
      eq(agentDailyLogs.userId, userId),
      eq(agentDailyLogs.logDate, date),
    ),
  });
