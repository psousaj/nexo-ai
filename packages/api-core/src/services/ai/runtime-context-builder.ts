...
export async function buildRuntimeContext(request: RuntimeContextBuilderRequest): Promise<RuntimeContextBuilderResult> {
	const { assistantName, systemPrompt } = await resolvePromptContext(request.userId, request.sessionKey, request.availableTools);

	const history = await conversationService.getHistory(request.conversationId, request.historyLimit ?? 10);
	const historyBlocks: RuntimeHistoryBlock[] = history
		.filter((item: any): item is { role: 'user' | 'assistant'; content: string } => {
	...
	const openAIMessages: OpenAI.Chat.ChatCompletionMessageParam[] = coreMessages
		.filter((message): message is Extract<CoreMessage, { role: 'user' | 'assistant' }> => message.role === 'user' || message.role === 'assistant')
		.map((message) => ({
			role: message.role,
...
async function resolvePromptContext(
	...
	if (sessionKey) {
		...
		const basePrompt = buildAgentPrompt({ assistantName, availableTools }).system;
		return {
			assistantName,
			systemPrompt: agentContext.systemPrompt ? `${basePrompt}\n\n# PERSONALIZED CONTEXT\n${agentContext.systemPrompt}` : basePrompt,
		};
	}
