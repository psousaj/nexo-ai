...
export class EvolutionAdapter implements MessagingProvider {
	...
	parseIncomingMessage(payload: any): IncomingMessage | null {
		...
		const timestampRaw = Number(message?.messageTimestamp || Math.floor(Date.now() / 1000));
		...
		} else if (trimmedText.startsWith('select_') || trimmedText.startsWith('confirm_') || trimmedText.startsWith('choose_')) {
	...
	async sendMessageWithButtons(chatId: string, text: string, buttons: any[]): Promise<void> {
		...
		try {
			...
			loggers.webhook.warn({ err: error }, 'Falha ao enviar lista interativa via Evolution; aplicando fallback em texto');
