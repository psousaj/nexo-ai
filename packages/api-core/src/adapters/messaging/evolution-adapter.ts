...
function flattenButtons(
  ...
  const flattened = buttons.flatMap((item: any) =>
...
export class EvolutionAdapter implements MessagingProvider {
  ...
  parseIncomingMessage(payload: any): IncomingMessage | null {
    ...
    const phoneNumber = remote.isPn
      ? remote.identifier
      : participant.isPn
        ? participant.identifier
        : undefined;
    const userId =
      isGroup && participant.identifier
        ? participant.identifier
        : remote.identifier;
    ...
    return {
      ...
      metadata: {
        ...
        participantJid: key.participant,
        providerPayload: JSON.parse(JSON.stringify(payload)) as Record<
          string,
          unknown
        >,
      },
